...
/**
 * yyyy-MM-dd hh:mm:ss 시간을 unix 시간으로 변환
...
/**
 * 날짜비교(now 보다 afterTime 이 동일이거나 과거이면 false)
 * @param now
 * @param afterTime
 * @returns {boolean}
 */
function compareTime(now, afterTime) {
	var boolean = true;
	if ((now > afterTime) || (now == afterTime)) {
		boolean = false;
	}  
	return boolean;
}
...
//셀렉트박스 선택 제어
function gfn_selectbox_selected(tag_id, select_value, boolean) {
    $("#" + tag_id).val(select_value).attr("selected", boolean);
}
////////////////////////////////////////////////////////////////////////////////
